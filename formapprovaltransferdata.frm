...
Begin VB.Form formapprovaltransferdata 
   ...
   Begin VB.ComboBox cmbsorted 
      ...
      Left            =   8640
      List            =   "formapprovaltransferdata.frx":0007
      TabIndex        =   10
      Top             =   2880
      Width           =   1335
   End
   ...
   Begin VB.ComboBox cmbapprove 
      Height          =   315
      ItemData        =   "formapprovaltransferdata.frx":0011
      Left            =   8640
      List            =   "formapprovaltransferdata.frx":0018
      TabIndex        =   7
      Top             =   2040
      Width           =   1335
   End
...
'    sQuery = sQuery + " left join (select max(paydate) as paydate ,custid from tbllunas group by 2) tbllunas on tian.custid = tbllunas.custid where 1 = 1 and y_n = 1"
    ...
    sQuery = "select custid,agentlama,agentbaru, tllama, z.team tlbaru, tanggalupload, pengupload, recsource, b_d, f_cek_new, tujapproval, y_n, paydate from ( "
    sQuery = sQuery + " select custid, agentlama, agentbaru, y.team tllama, tanggalupload, pengupload, recsource, b_d, f_cek_new, tujapproval, y_n, paydate  from "
    ...
    If RS_Lv.RecordCount > 0 Then
        ...
        Do Until RS_Lv.EOF
            ...
            listItem.SubItems(3) = Trim(cnull(RS_Lv("agentlama")))
            listItem.SubItems(4) = Trim(cnull(RS_Lv("agentbaru")))
...
'            Call cetak
    ...
    For W = 1 To lvapprovaltransferdata.ListItems.Count
        If lvapprovaltransferdata.ListItems(W).Checked = True Then
            ...
            CMDSQL2 = "update mgm set agent = '" & lvapprovaltransferdata.ListItems(W).ListSubItems(4) & "'"
