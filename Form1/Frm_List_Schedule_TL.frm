...
Begin VB.Form frm_list_schedule_tl 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Tab(1).Control(0)=   "TxtJmlDataCurrent"
      Tab(1).Control(1)=   "CmdUncekAll"
      Tab(1).Control(2)=   "CmdCekAll"
      Tab(1).Control(3)=   "CmdRefreshCurrent"
      Tab(1).Control(4)=   "CmdRelease"
      Tab(1).Control(5)=   "LvLockAccCurrent"
      Tab(1).Control(6)=   "Label4"
      Tab(1).ControlCount=   7
      TabCaption(2)   =   "Log lock account"
      TabPicture(2)   =   "Frm_List_Schedule_TL.frx":0038
      Tab(2).ControlEnabled=   0   'False
      Tab(2).Control(0)=   "TxtJmlDataLog"
      Tab(2).Control(1)=   "CmdRefreshLog"
      Tab(2).Control(2)=   "LvLockAccLog"
...
Private Sub IsiLockLog()
    ...
    qins = "insert into tbltemplockacc_log select * from tbltemplockacc_log where date_lock < to_char(now() -  interval '3 month','yyyy-mm-01')::date;" & vbCrLf
    qdel = "delete from tbltemplockacc_log where date_lock < to_char(now() -  interval '3 month','yyyy-mm-01')::date;"
    
    M_OBJCONN.Execute qins & qdel
...
Private Sub IsiLockCurrent()
    ...
    Else
        cmdsql = "select * from tbltemplockacc_current order by account_lock,start_lock asc"
    End If
