...
'
                Set m_cari = M_DATA.QUERY_SEARCH_CONDITION_mgm(M_OBJCONN, NAMACUST, NamaAgent, DATASOURCE, TGLLAHIR, _
                                                             OFFPHONE, OFFPHONE2, HOMEPHONE, HOMEPHONE2, MOBILEPHONE, _
                                                            MOBILEPHONE2, FAXPHONE, FAXPHONE2, MDIForm1.Text3.Text, Lcustid, Bloked, lLastCallDate, lStatusCek, sPending, FCEKSTATUS, WO_DATE, strverify, strapprovel, strapprovelyet, strReject, strSukses, Bloked, BlokedEntry, blokeddatamarkup, nmagentprev, "", BlokedPayment)
        ...
        If UCase(MDIForm1.Text2.Text) = "AGENT" Or _
           UCase(MDIForm1.Text2.Text) = "TEAMLEADER" Then
            ...
            Else
                ...
                Set M_ObjrsCekAkses = Nothing
                'delete dulu deh datanya
                Cmdsql = "delete from tbl_distribusi_account where custid='"
                Cmdsql = Cmdsql & CStr(LstVwSearchMgm.SelectedItem.SubItems(1)) & "' and agent='"
...
'        FrmCustIdTransfer.Show
        If UCase(Left(MDIForm1.Text2.Text, 5)) = "ADMIN" Then
            b_pindah = True
            FrmCustIdTransfer.Show
        ElseIf UCase(MDIForm1.Text2.Text) = "SUPERVISOR" Then
...
Private Sub CariDataAll()
    ...
    If UCase(MDIForm1.Text2.Text) = "AGENT" Or _
        UCase(MDIForm1.Text2.Text) = "TEAMLEADER" Then
        Cmdsql = "select f_akses_all_acc from usertbl where userid='"
        Cmdsql = Cmdsql + MDIForm1.Text1.Text + "'"
    ...
    If AksesAllAcc = "1" Then
        ...
        Cmdsql = Cmdsql + " (select custid from tbl_distribusi_account "
        Cmdsql = Cmdsql + " where agent='"
        Cmdsql = Cmdsql + MDIForm1.Text1.Text + "' "
        Cmdsql = Cmdsql + " and waktu_akhir > now() and  "
        Cmdsql = Cmdsql + " waktu_awal <= now()) and agent='AKSESALL'"
    Else
        Exit Sub
    End If
...
SorryLompat:
    ...
    Else
        ...
        M_ObjrsCekStatus.Open Cmdsql, M_OBJCONN, adOpenDynamic, adLockOptimistic, adCmdText
        If M_ObjrsCekStatus.RecordCount = 0 Then
            Cmdsql = "update usertbl set f_akses_all_acc=null,f_pesanresetauto='1' where userid='"
            Cmdsql = Cmdsql + MDIForm1.Text1.Text + "'"
            M_OBJCONN.Execute Cmdsql
            AksesAllAcc = ""
        End If
