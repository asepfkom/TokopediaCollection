...
Private Sub Command1_Click()
    ...
    M_OBJCONN.Execute "INSERT INTO tblreport_komisi(custid,promisedate,agent,tglinput,promisepay,tgl_report) " & _
                    "SELECT b.* FROM (SELECT custid,promisedate as tgl_max,agent FROM tblnegoptp_log  " & _
                    " WHERE (date_part('month',tglinput)=" & sql_bulan & " AND date_part('year',tglinput)=" & sql_tahun & ") " & _
                    " AND (date_part('month',promisedate)=" & sql_bulan & " AND date_part('year',promisedate)=" & sql_tahun & ") " & _
                    " GROUP BY custid,agent,promisedate) a, (SELECT custid,promisedate,agent,tglinput,promisepay,to_date('" & Format(tgl_lap, "yyyy-mm-01") & "','YYYY-MM-DD') " & _
                    " FROM tblnegoptp_log  WHERE date_part('month',tglinput)=" & sql_bulan & " AND date_part('year',tglinput)=" & sql_tahun & " ) b WHERE a.custid=b.custid AND a.agent=b.agent AND a.tgl_max=b.promisedate;"
    ...
    If rs_temp.RecordCount > 0 Then
        ...
        Do Until rs_temp.EOF
            ...
            sql_str = "SELECT sum(a.payment) as total_bayar,max(a.paydate) as tgl_akhir FROM (SELECT * FROM tbllunas WHERE date_part('month',paydate)=" & sql_bulan & " AND date_part('year',paydate)=" & sql_tahun & " ) a " & _
                        " WHERE a.custid='" & cnull(rs_temp!CustId) & "' AND a.agent='" & cnull(rs_temp!agent) & "' AND date(a.paydate)>='" & Format(rs_temp!PromiseDate, "yyyy-mm-dd") & "'"
            ...
            M_OBJCONN.Execute "UPDATE tblreport_komisi SET payment=" & temp_payment & ",paydate=" & temp_paydate & ",f_kept='" & f_kept & "' WHERE id=" & cnull(rs_temp!ID) & ""
    ...
    ' JUMLAH PTP
    sql_str = sql_str + " (SELECT agent,count(custid) as jml_PTP FROM tblnegoptp_log WHERE date_part('month',promisedate)=" & sql_bulan & " AND date_part('year',promisedate)=" & sql_tahun & " GROUP BY agent) s ON r.userid=s.agent ) t LEFT JOIN "
    ' KEPT
    sql_str = sql_str + " (SELECT agent,count(custid) as jml_kept,sum(payment) as keptamount FROM tblreport_komisi WHERE date_part('month',tgl_report)=" & sql_bulan & " AND date_part('year',tgl_report)=" & sql_tahun & " AND f_kept='KEPT' GROUP BY agent) u ON t.userid=u.agent ) v LEFT JOIN "
    ' BP
    sql_str = sql_str + " (SELECT agent,count(custid) as broken FROM tblreport_komisi WHERE date_part('month',tgl_report)=" & sql_bulan & " AND date_part('year',tgl_report)=" & sql_tahun & " AND f_kept='BP' GROUP BY agent) w ON v.userid=w.agent ) x LEFT JOIN "
    ' RPC 1
