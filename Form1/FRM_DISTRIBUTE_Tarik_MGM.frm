...
Begin VB.Form FRM_distribute_tarik_MGM 
   BackColor       =   &H80000004&
   ...
   ClientTop       =   330
   ClientWidth     =   9390
   Icon            =   "FRM_DISTRIBUTE_Tarik_MGM.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   7320
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "FRM_DISTRIBUTE_Tarik_MGM.frx":0442
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "Frame2"
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "Distribusi"
      TabPicture(1)   =   "FRM_DISTRIBUTE_Tarik_MGM.frx":045E
   ...
   End
End
Attribute VB_Name = "FRM_distribute_tarik_MGM"
...
Text2.Text = 0
        JUMLAH = HITUNG_TEMPCUST_CC(M_OBJCONN, "CUSTID IN(SELECT CUSTID FROM MGM WHERE AGENT ='NELLY' and RECSOURCE = '" + Combo1(0).Text + "') ")
    ...
    Case 0
        ...
        ListView1.ListItems(i).Selected = True
            If ListView1.ListItems(i).SubItems(2) <> "0" Then
                ...
                cmdsql = "Update MGM SET AGENT = " + "'" + ListView1.ListItems(i).Text + "'"
                cmdsql = cmdsql + " ,TGLSTATUS =" + "'" + Format(MDIForm1.TDBDate1.Value, "YYYY/MM/DD HH:NN") + "'"
                'cmdsql = cmdsql + " ,TGLDISTRIBUSI =" + "'" + Format(MDIForm1.TDBDate1.Value, "YYYY/MM/DD HH:NN") + "'"
                ...
                cmdsql = cmdsql + " CUSTID FROM MGM where AGENT ='NELLY' and recsource ='" + Combo1(0).Text + "') "
...
Dim LISTITEM As LISTITEM
    ...
    Call header
    Call header1
    Set LISTITEM = ListView1.ListItems.ADD(, , FRM_SETUSER_Tarik_mgm.Combo1(0).Text)
        LISTITEM.SubItems(1) = FRM_SETUSER_Tarik_mgm.Combo1(1).Text
        LISTITEM.SubItems(2) = 0
        LISTITEM.SubItems(3) = Format(MDIForm1.TDBDate1.Text, "yyyymmdd") & Format(Now, "hhmm")
      
    Set m_objrs = QUERY_USER_ACC(M_RPTCONN, FRM_SETUSER_Tarik_mgm.Combo1(0).Text)
...
Set m_objrs = Nothing
'Call QUERY_SPV
Set m_objrs = QUERY_SPV(M_OBJCONN, " SPVCODE = '" + FRM_SETUSER_Tarik_mgm.Combo1(0).Text + "'")
If m_objrs.RecordCount <> 0 Then
...
cmdsql = "SELECT COUNT(CUSTID) AS JML FROM MGM"
 If Len(M_WHERE) <> 0 Then
    cmdsql = cmdsql + " WHERE " + M_WHERE
 End If
