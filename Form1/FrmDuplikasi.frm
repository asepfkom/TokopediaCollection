...
Begin VB.Form FrmDuplikasi 
   Caption         =   "Duplikasi Data"
   ClientHeight    =   7080
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   14190
   Icon            =   "FrmDuplikasi.frx":0000
   LinkTopic       =   "Form2"
   ScaleHeight     =   7080
   ScaleWidth      =   14190
   ...
   End
End
Attribute VB_Name = "FrmDuplikasi"
...
Private Sub CmdCloseData_Click()
    ...
    If Len(TxNewLeadsId.Text) < 3 Then
        MsgBox "Pilih New Leads Yang Akan Di Close", vbCritical + vbOKOnly, "Telegrandi"
        Exit Sub
    End If
    ...
    CMDSQL = "Select * from TBL_DUPLIKASI WHERE CUSTIDBARU ='" + TxNewLeadsId.Text + "'"
...
Private Sub CmdReleaseData_Click()
    ...
    If Len(TxNewLeadsId.Text) < 10 Then
        MsgBox "Pilih New Leads Yang Akan Di Release", vbCritical + vbOKOnly, "Telegrandi"
        Exit Sub
    End If
    ...
    If m_rs.RecordCount <> 0 Then
        ...
        If IsNull(m_rs!BIRTHD) = False Then
            CMDSQL = CMDSQL + "'" + Format(m_rs!TGLENTRY, "yyyy/mm/dd") + "',"
        End If
        CMDSQL = CMDSQL + "'" + m_rs!recsourcebaru + "')"
...
m_cek1.Open "Select * from VW_DUPLIKASI where AGENTBARU BETWEEN '" + kriteria1 + "' AND '" + kriteria2 + "' AND sts = 0", M_OBJCONN, adOpenDynamic, adLockOptimistic, adCmdText
i = 0
