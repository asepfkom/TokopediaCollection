...
Begin VB.Form frm_transferdata2 
   ...
   End
End
Attribute VB_Name = "frm_transferdata2"
...
Private Sub Command1_Click()
    ...
    qc = qc + "update mgm set agent = a.agent from tbltemp_transfer a where mgm.custid = a.custid;"
...
Private Sub Command2_Click()
    ...
    qs = "select * from information_schema.columns where table_name = 'tbltemp_transfer'"
    Set M_Objrs = New ADODB.Recordset
    M_Objrs.CursorLocation = adUseClient
    M_Objrs.Open qs, M_OBJCONN, adOpenDynamic, adLockOptimistic
        
    If M_Objrs.RecordCount = 0 Then
        qc = "create table tbltemp_transfer ( id serial, custid varchar, agent varchar );"
        M_OBJCONN.execute qc
    End If
        qd = "delete from tbltemp_transfer;"
        ...
        While Not rs.EOF
            ...
            str_sql = "INSERT INTO tbltemp_transfer (custid,agent) Values ( '" + CStr(rs(0)) + "', '" + rs(1) + "' );"
        ...
        qs = "select mgm.custid,mgm.name,mgm.agent as agentlama, tbltemp_transfer.agent as agentbaru from mgm,tbltemp_transfer  where mgm.custid = tbltemp_transfer.custid"
        Set M_Objrs = New ADODB.Recordset
        M_Objrs.CursorLocation = adUseClient
        M_Objrs.Open qs, M_OBJCONN, adOpenDynamic, adLockOptimistic
