...
Begin VB.Form Form_upload_CPA 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cbomap 
         Height          =   315
         ItemData        =   "frm_upload_CPA.frx":0000
         Left            =   1380
         List            =   "frm_upload_CPA.frx":0002
         TabIndex        =   10
         Top             =   270
         Width           =   2595
      End
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(3)   =   "frm_upload_CPA.frx":0058
      ...
      TabPicture(4)   =   "frm_upload_CPA.frx":0074
   ...
   Begin VB.Image Image2 
      Height          =   465
      Left            =   0
      Picture         =   "frm_upload_CPA.frx":0090
      Stretch         =   -1  'True
      Top             =   0
      Width           =   17220
   End
   ...
   Begin VB.Image Image1 
      ...
      Index           =   0
      Left            =   30
      Picture         =   "frm_upload_CPA.frx":2C75
      Stretch         =   -1  'True
      Top             =   30
      Width           =   420
   End
End
Attribute VB_Name = "Form_upload_CPA"
...
Private Sub cmdcreatemap_Click()
   Form_setting_upload_cpa.Show 1
End Sub
Public Sub loadCboMap()
    cbomap.clear
    ssql = "select DISTINCT(kode_source) from tbl_setting_upload_cpa  where (kode_source is not null or kode_source<>'')"
    Set M_Objrs = New ADODB.Recordset
        M_Objrs.CursorLocation = adUseClient
...
Dim list As listItem
    ...
    sStrsql = sStrsql + "  select field_source,field_destination from tbl_setting_upload_cpa where kode_source='" + xCodeMap + "' ) "
...
Public Function cekmapping_excel() As Boolean
    ...
    Strsql = Strsql + " SELECT column_name as nama_kolom From information_schema.Columns WHERE table_name='tbl_upload_cpa'"
    ...
    Strsql = Strsql + " (   select field_source,field_destination from tbl_setting_upload_cpa  where kode_source='" + cbomap.text + "')"
    ...
    Strsql = Strsql + " Left Join ( select * from tbl_temp_field_cpa  ) as tbldua   on tblsatu.field_destination=tbldua.nama_field"
    Set M_Objrs = New ADODB.Recordset
        M_Objrs.CursorLocation = adUseClient
        M_Objrs.Open Strsql, M_OBJCONN, adOpenDynamic, adLockOptimistic
       If M_Objrs.RecordCount = 0 Then
