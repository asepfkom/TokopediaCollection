...
Begin VB.Form Form_setting_upload_cpa 
   Caption         =   "Setup Map CPA"
   ClientHeight    =   9480
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   13065
   ...
   Begin VB.Frame Frame2 
      ...
      Begin TabDlg.SSTab SSTab1 
         ...
         TabPicture(0)   =   "form_setting_upload_CPA .frx":0000
         Tab(0).ControlEnabled=   0   'False
         Tab(0).Control(0)=   "fx_mapping"
         Tab(0).Control(1)=   "Cboexecelmap"
         Tab(0).ControlCount=   2
         TabCaption(1)   =   "History user "
         TabPicture(1)   =   "form_setting_upload_CPA .frx":001C
   ...
   Begin VB.Image Image1 
      ...
      Index           =   0
      Left            =   0
      Picture         =   "form_setting_upload_CPA .frx":0038
      Stretch         =   -1  'True
      Top             =   0
      Width           =   420
   End
   ...
   Begin VB.Image Image2 
      Height          =   465
      Left            =   0
      Picture         =   "form_setting_upload_CPA .frx":0B42
      Stretch         =   -1  'True
      Top             =   0
      Width           =   15360
   End
End
Attribute VB_Name = "Form_setting_upload_cpa"
...
ssql = "SELECT * FROM  tbl_setting_upload_cpa WHERE KODE_SOURCE='" + cbomapsource + "'"
...
If bBool = False Then
    ...
    sStrsql = sStrsql + " SELECT column_name as nama_kolom From information_schema.Columns WHERE table_name='tbl_upload_cpa'"
...
Else
   sStrsql = " select nama_kolom,field_destination from ( "
    ...
    sStrsql = sStrsql + "  select field_source,field_destination from tbl_setting_upload_cpa where kode_source='" + cbomapsource.Text + "' ) "
...
Dim rsTemp As New ADODB.Recordset
            ...
            ssql = "DELETE FROM tbl_setting_upload_cpa WHERE kode_source ='" & cbomapsource.Text & "'"
            M_OBJCONN.Execute (ssql)

            For brs = 1 To fx_mapping.Rows - 1
                If fx_mapping.TextMatrix(brs, 2) <> vbNullString Then
                    ssql = "INSERT INTO tbl_setting_upload_cpa (kode_source,nama_source,table_source ,location_source,field_source , field_destination) "
                    ssql = ssql & "VALUES "
                    With fx_mapping
    ...
    Strsql = "insert into tbl_hst_setting_upload_cpa (user_input,action_user) values ('" + MDIForm1.Text1.Text + "','" + sAction + "') "
...
Dim no As Double
sStrsql = "select * from tbl_hst_setting_upload_cpa"
Set M_Objrs = New ADODB.Recordset
    M_Objrs.CursorLocation = adUseClient
    M_Objrs.Open sStrsql, M_OBJCONN, adOpenDynamic, adLockOptimistic
    no = 0
...
cbomapsource.CLEAR
 ssql = "select DISTINCT(kode_source) from tbl_setting_upload_cpa"
 Set M_Objrs = New ADODB.Recordset
 M_Objrs.CursorLocation = adUseClient
 M_Objrs.Open ssql, M_OBJCONN, adOpenDynamic, adLockOptimistic
 While Not M_Objrs.EOF
