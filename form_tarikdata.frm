...
Begin VB.Form form_tarikdata 
   Caption         =   "Update Status PTP "
   ClientHeight    =   6735
   ClientLeft      =   60
   ClientTop       =   405
   ClientWidth     =   5490
   ...
   End
End
Attribute VB_Name = "form_tarikdata"
...
Private Sub Command1_Click()
    ...
    qc = "create table backuptblupdate_sts_payment___" & abc & " as (select mgm.custid,mgm.status_ptp as status_ptp_lama, a.status_ptp as status_ptp_baru from mgm, tblupdate_sts_payment a where mgm.custid = a.custid);" & vbCrLf
    qc = qc + "alter table backuptblupdate_sts_payment___" & abc & " add column distributeby varchar ;" & vbCrLf
    qc = qc + "alter table backuptblupdate_sts_payment___" & abc & " add column tanggal timestamp without time zone default now();" & vbCrLf
    qc = qc + "update backuptblupdate_sts_payment___" & abc & " set distributeby = '" & MDIForm1.Text1.text & "';" & vbCrLf & vbCrLf
    qc = qc + "update mgm set status_ptp = a.status_ptp from tblupdate_sts_payment a where mgm.custid = a.custid;"
    'qc = qc + "insert into tblupdate_sts_payment_log(custid,status_ptp,data_update)select custid,status_ptp,data_update from tblupdate_sts_payment where custid = tblupdate_sts_payment.custid;"
    ...
    MsgBox "Update Status PTP Success"
...
Private Sub Command2_Click()
    ...
    qs = "select * from information_schema.columns where table_name = 'tblupdate_sts_payment'"
    Set M_Objrs = New ADODB.Recordset
    M_Objrs.CursorLocation = adUseClient
    M_Objrs.Open qs, M_OBJCONN, adOpenDynamic, adLockOptimistic
        
    If M_Objrs.RecordCount = 0 Then
        qc = "create table tblupdate_sts_payment ( id serial, custid varchar, status_ptp varchar );"
        'custid,name,gender,Product_type,due_date,days_past_due,loan_amount,installment,tenure,installment_no,Interest,AdminFee,Interest_idr,AdminIDR,TotalPrinInt,Daily_Late_Fees_Daily_Late(H+3),last_payment_date,mobile_phone,actual_province,actual_district,econ_name,econ_number,Agent,Call_Date,Status_Call,PTP_Date,Remark,Team_Leader,Status_PTP,Call_Hangup_By_System,Call_Connected

        M_OBJCONN.execute qc
    End If
        qd = "delete from tblupdate_sts_payment;"
        ...
        While Not rs.EOF
            ...
            str_sql = "INSERT INTO tblupdate_sts_payment (custid,status_ptp) Values ( '" + CStr(rs(0)) + "', '" + rs(1) + "' );"
        ...
        qs = "select mgm.custid,mgm.name, tblupdate_sts_payment.status_ptp as status_ptp from mgm,tblupdate_sts_payment  where mgm.custid = tblupdate_sts_payment.custid"
        Set M_Objrs = New ADODB.Recordset
        M_Objrs.CursorLocation = adUseClient
        M_Objrs.Open qs, M_OBJCONN, adOpenDynamic, adLockOptimistic
...
'                listItem.SubItems(3) = cnull(M_Objrs("agentbaru"))
                listItem.SubItems(2) = cnull(M_Objrs("status_ptp"))
            M_Objrs.MoveNext
        Wend
